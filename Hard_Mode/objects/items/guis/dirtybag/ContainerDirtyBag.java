...
import Hard_Mode.objects.items.guis.slots.ItemExcludeSlot;
...
public class ContainerDirtyBag extends Container{
    ...
    public ContainerDirtyBag(IInventory playerInventory, IInventory chestInventory, EntityPlayer player)
    {
        ...
        for (int j = 0; j < this.numRows; ++j)
        {
            for (int k = 0; k < 9; ++k)
            {
                this.addSlotToContainer(new ItemExcludeSlot(chestInventory, k + j * 9, 8 + k * 18, 18 + j * 18, ItemInit.DIRTY_BAG));
            }
        }
